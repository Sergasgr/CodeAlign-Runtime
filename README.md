...
Cuantización de Activaciones (On-the-fly): Si estuvieras cuantizando los datos que entran al modelo (el prompt del usuario) o los resultados intermedios de las capas, sí, tendrías que hacerlo en la GPU paralelizado porque eso suma latencia en cada token generado.

Cuantización de Pesos (Ahead-of-Time): Lo que estás haciendo ahora es comprimir los pesos estáticos del modelo. En un sistema de producción real (como llama.cpp), esta función no se ejecuta cuando el usuario pide texto. Se ejecuta semanas antes en un ordenador, se guarda el resultado en un archivo .gguf en el disco duro, y el motor de inferencia simplemente lee ese archivo ya comprimido.